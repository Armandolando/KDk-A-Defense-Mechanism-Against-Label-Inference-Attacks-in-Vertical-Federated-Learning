...
class TopModelForCifar10Plus(nn.Module):
    def __init__(self):
        super(TopModelForCifar10Plus, self).__init__()
        self.fc1top = nn.Linear(20, 128)
        self.fc2top = nn.Linear(128, 64)
        self.fc3top = nn.Linear(64, 32)
        self.fc4top = nn.Linear(32, 10)
        self.bn0top = nn.BatchNorm1d(20)
        self.bn1top = nn.BatchNorm1d(128)
        self.bn2top = nn.BatchNorm1d(64)
        self.bn3top = nn.BatchNorm1d(32)
...
class TopModel:
    ...
    def get_model(self, fine_tune=False):
        ...
        elif self.dataset_name == 'CIFAR10' and not fine_tune:
            return TopModelForCifar10()
        elif self.dataset_name == 'CIFAR10' and fine_tune:
            return TopModelForCifar10Plus()
        elif self.dataset_name == 'CIFAR100':
...
def update_top_model_one_batch(optimizer, model, output, batch_target, loss_func):
    ...
    #loss_func = LabelSmoothingLoss(epsilon=0.5, num_classes=batch_target.shape[1])
    #loss = loss_func(output, batch_target)
    #print(batch_target)
    optimizer.zero_grad()
    loss.backward()
